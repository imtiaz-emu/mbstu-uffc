module FixturesSupport
  ...
  # GET /ohid/1 or /ohid/1.json
  # GET /khaled/1 or /khaled/1.json
  def show
    set_fixture
  ...
  def fixture_data
    all_fixtures = params[:team_name].present? ? JSON.load(File.read(Rails.root.join('lib', "bor_#{params[:team_name]}.json"))) : all_fixtures()
